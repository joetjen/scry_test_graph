defmodule Scry.Test.Graph.MixProject do
  use Mix.Project
  ...
  defp deps do
    [
      # === SCRY CORE / SCRY GRAPH ===
      # Local path dependencies, not Hex version constraints, since
      # neither is published to Hex yet. Both real, not test-only:
      # this package parses query text through Scry.Graph.parse/1 and
      # drives Scry.Graph.Executor.run/3 directly.
  ...
  defp description do
    "Shared test fixtures for scry_graph: one seed dataset servable through Scry.Test.Graph." <>
      "Conn's own graph/0 constructor, plus scry_core's own mix scry.query/scry.iex " <>
      "(configured here) for exercising it. Not a cross-backend parity suite -- scry_graph " <>
      "has exactly one executor, no interchangeable EngineBehaviour backends to compare."
  end

  defp package do
    [
      licenses: ["MIT"],
      links: %{"GitHub" => "https://github.com/joetjen/scry_test_graph"},
  ...
  defp docs do
    [
      main: "readme",
      source_url: "https://github.com/joetjen/scry_test_graph",
